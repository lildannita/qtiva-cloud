SHELL := /usr/bin/env bash
.DEFAULT_GOAL := help

COMPOSE_FILE := docker-compose.dev.yml
PG_CONTAINER := qtiva-postgres
VERSION ?= dev

BIN_DIR := bin
MANAGER_BIN := $(BIN_DIR)/qtiva-manager
AGENT_BIN   := $(BIN_DIR)/qtiva-agent

.PHONY: help env up down restart ps logs psql migrate tidy build clean \
        run-manager run-agent health-manager health-agent

help: ## Показать список команд
	@grep -E '^[a-zA-Z0-9_-]+:.*?## .*$$' $(MAKEFILE_LIST) \
	| awk 'BEGIN {FS = ":.*?## "}; {printf "\033[36m%-18s\033[0m %s\n", $$1, $$2}'

env: ## Создать .env из .env.example (если .env отсутствует)
	@if [ -f .env ]; then \
		echo ".env уже существует"; \
	else \
		cp .env.example .env && echo "Создан .env (скопирован из .env.example)"; \
	fi

up: env ## Поднять dev-инфраструктуру (Postgres)
	docker compose -f $(COMPOSE_FILE) up -d

down: ## Остановить dev-инфраструктуру
	docker compose -f $(COMPOSE_FILE) down

restart: down up ## Перезапустить dev-инфраструктуру

ps: ## Показать статус контейнеров compose
	docker compose -f $(COMPOSE_FILE) ps

logs: ## Логи Postgres
	docker logs -f $(PG_CONTAINER)

psql: env ## Зайти в psql внутри контейнера (использует переменные из .env)
	@set -a && source .env && set +a && \
	docker exec -it $(PG_CONTAINER) psql -U $$POSTGRES_USER -d $$POSTGRES_DB

migrate: env ## Применить миграции
	@chmod +x ./scripts/migrate_up.sh
	./scripts/migrate_up.sh

tidy:
	go mod tidy

build: ## Собрать бинарники
	@chmod +x ./scripts/build.sh
	@VERSION="$(VERSION)" ./scripts/build.sh

clean: ## Удалить ./bin
	rm -rf $(BIN_DIR)

run-manager: ## Запустить manager
	@if [ ! -f "$(MANAGER_BIN)" ]; then \
		echo "Исполняемый файл $(MANAGER_BIN) не найден"; \
		exit 1; \
	fi
	./$(MANAGER_BIN) serve

run-agent: ## Запустить agent
	@if [ ! -f "$(AGENT_BIN)" ]; then \
		echo "Исполняемый файл $(AGENT_BIN) не найден"; \
		exit 1; \
	fi
	./$(AGENT_BIN) serve

health-manager: ## Проверить /health у manager
	curl -sS http://127.0.0.1:8080/health

health-agent: ## Проверить /health у agent
	curl -sS http://127.0.0.1:8090/health
